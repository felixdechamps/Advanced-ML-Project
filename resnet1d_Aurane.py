...
class MyMaxPool1dPadSame(nn.Module):
    ...
    def forward(self, x):
        ...
        # compute pad shape
        in_dim = net.shape[-1]  # length of the signal L_in
        # out_dim equals in_dim
        out_dim = (in_dim + self.stride - 1) // self.stride
        # so in this case padding p equals this (classic formula)
        p = max(0, (out_dim - 1) * self.stride + self.kernel_size - in_dim)
        pad_left = p // 2
        pad_right = p - pad_left
        net = F.pad(net, (pad_left, pad_right), "constant", 0)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.out_channels != self.in_channels:
            # because F.pad adds zeros on the last dimension
            identity = identity.transpose(-1, -2)
            # nb of channels to add in front of existing channels (half the difference)
            ch1 = (self.out_channels-self.in_channels)//2
            # nb of channels to add behind existing channels
