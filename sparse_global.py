'cf jfrankle/open_lth//blob/master/pruning/sparse_global.py'

import numpy as np
from mask import Mask

def prune(pruning_fraction, layers_to_ignore, trained_model, current_mask: Mask = None):
    '''
    ...
    prunable_tensors = set(trained_model.prunable_layers_names)
    if layers_to_ignore:
        prunable_tensors -= set(layers_to_ignore.split(','))

    # Get the model weights.
    weights = {k: v.clone().cpu().detach().numpy() for k, v in trained_model.state_dict().items() if k in prunable_tensors}

    # Create a vector of all the unpruned weights in the model.
    weight_vector = np.concatenate([v[current_mask[k] == 1] for k, v in weights.items()])
    threshold = np.sort(np.abs(weight_vector))[number_of_weights_to_prune]

    new_mask = Mask({k: np.where(np.abs(v) > threshold, current_mask[k], np.zeros_like(v)) for k, v in weights.items()})
