...
def load_ecg_mat(ecg_file):
    """Returns the values for a ecg file in .mat"""
    return sio.loadmat(ecg_file)['val'].squeeze()


def load_all(data_path):
    """ create a dataset consisting of (ecg file path, list of labels)
        one label every 256 measures"""
...
def split(dataset, dev_frac):
    """split the dataset in train and dev parts"""
...
def make_json(save_path, dataset):
    """transform dataset into a json file"""
